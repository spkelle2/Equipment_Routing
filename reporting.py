...
def equipment_usage_analysis(demand_df, directory_name, variation):
...
def equipment_graph_maker(demand_df, variation, directory_name, plotlist):
    ...
    usage = equipment_usage_analysis(demand_df, directory_name, variation)
    ...
    fig.suptitle('Equipment Set Utilization by Year for %s variation' % variation)
    ...
    fig.suptitle('Cumulative Machine Utilization by Year for %s variation' % variation)
...
def hauler_graph_maker(hours_df, index, plotlist, directory_name, variation):
...
def make_report(data, variation, fixed_parameters):
    ...
    plotlist = equipment_graph_maker(demand_df, variation, directory_name, plotlist)

    # make graphs detailing how much each hauler works each day
    for index in hauler_summary.index:
        plotlist = hauler_graph_maker(hours_df, index, plotlist, directory_name, variation)

    # html template for our report
    env = Environment(loader=FileSystemLoader('.'))
    template = env.get_template('myreport.html')

    # variables to be placed into html via jinja templating
    template_vars = {
        'title' : '%s' % variation,
